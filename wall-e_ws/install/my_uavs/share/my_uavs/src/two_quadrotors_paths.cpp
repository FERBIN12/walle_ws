...
Transformation between frames are send to a correct visulization in Rviz.
Robots paths are published on its corresponding "path" topic.
...
$ ros2 launch my_uavs two_ign_world_launch.py
	# Launchs two simulated quadrotors in Ignition, runs ros_gz_bridge, opens Rviz and runs the static transform publisher node

$ ros2 run my_uavs two_quadrotors_paths r1 r2
...
{
  public:
    LFController( char * argv[] ) : Node("two_quadrotors_paths")
    {
      ns1_ = argv[1];
      ns2_ = argv[2];
      ...
      pub_path1_ = this->create_publisher<nav_msgs::msg::Path>(ns1_+"/path", 1);
      pub_path2_ = this->create_publisher<nav_msgs::msg::Path>(ns2_+"/path", 1);
      
      RCLCPP_INFO(this->get_logger(), "Press 'ESC' key to land the drones and to finish the node");
      RCLCPP_INFO(this->get_logger(), "Press 'c' key to clean robots trajectory");
    }
    ...
    void odom_callback1(const nav_msgs::msg::Odometry::SharedPtr msg)
    {
      ...
      tranStamp1_.header.stamp = this->get_clock()->now();
      tranStamp1_.header.frame_id = ns1_+frame_id_;
      tranStamp1_.child_frame_id = ns1_+child_frame_id_;

      tranStamp1_.transform.translation.x = r_pose1_.x;
      tranStamp1_.transform.translation.y = r_pose1_.y;
      tranStamp1_.transform.translation.z = r_pose1_.z;

      tranStamp1_.transform.rotation.x = q.x();
      tranStamp1_.transform.rotation.y = q.y();
      tranStamp1_.transform.rotation.z = q.z();
      tranStamp1_.transform.rotation.w = q.w();

      tf_broadcaster1_->sendTransform(tranStamp1_); // Send the transformation

      //Paths
      path_msg1_.header.frame_id = msg->header.frame_id;

      RCLCPP_INFO_ONCE(this->get_logger(), "[Robot 1] path frame_id: %s", path_msg1_.header.frame_id.c_str());
      
      robot_points1_.pose.position.x = r_pose1_.x;
      robot_points1_.pose.position.y = r_pose1_.y;
      robot_points1_.pose.position.z = r_pose1_.z;
    }
    ...
    void odom_callback2(const nav_msgs::msg::Odometry::SharedPtr msg)
    {
      ...
      //Create and fill out the transform broadcaster
      tranStamp2_.header.stamp = this->get_clock()->now();
      tranStamp2_.header.frame_id = ns2_+frame_id_;
      tranStamp2_.child_frame_id = ns2_+child_frame_id_;

      tranStamp2_.transform.translation.x = r_pose2_.x;
      tranStamp2_.transform.translation.y = r_pose2_.y;
      tranStamp2_.transform.translation.z = r_pose2_.z;

      tranStamp2_.transform.rotation.x = q.x();
      tranStamp2_.transform.rotation.y = q.y();
      tranStamp2_.transform.rotation.z = q.z();
      tranStamp2_.transform.rotation.w = q.w();

      tf_broadcaster2_->sendTransform(tranStamp2_); // Send the transformation

      //Paths
      path_msg2_.header.frame_id = msg->header.frame_id;

      RCLCPP_INFO_ONCE(this->get_logger(), "[Robot 2] path frame_id: %s", path_msg2_.header.frame_id.c_str());
      
      robot_points2_.pose.position.x = r_pose2_.x;
      robot_points2_.pose.position.y = r_pose2_.y;
      robot_points2_.pose.position.z = r_pose2_.z;
    }
    ...
    { 
      ...
      //Leader path
      path_msg1_.poses.push_back(robot_points1_); //Add the new point at the end of the list

      if(path_msg1_.poses.size() > 1600)
      {
        path_msg1_.poses.erase(path_msg1_.poses.begin()); //delete the oldest point
      }

      // if(key == 'c') //des_path_msg_.poses.size() > 4000
      // {
      //   key = -1;
      //   path_msg1_.poses.clear();
      //   RCLCPP_INFO(this->get_logger(), "Paths clean");
      // }

      pub_path1_->publish(path_msg1_); //Publish points on topic
    }
    ...
    { 
      ...
      //Follower path
      path_msg2_.poses.push_back(robot_points2_);

      if(path_msg2_.poses.size() > 1600)
      {
        path_msg2_.poses.erase(path_msg2_.poses.begin()); //delete the oldest point
      }

      pub_path2_->publish(path_msg2_); //Publish points on topic
}
  ...
  private:
    ...
    rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pub_path1_, pub_path2_;
    ...
    std::string frame_id_ = "/odom", child_frame_id_ = "/base_footprint", ns1_, ns2_;
    std::unique_ptr<tf2_ros::TransformBroadcaster> tf_broadcaster1_, tf_broadcaster2_;
    geometry_msgs::msg::TransformStamped tranStamp1_, tranStamp2_; 

    nav_msgs::msg::Path path_msg1_, path_msg2_;
    geometry_msgs::msg::PoseStamped robot_points1_, robot_points2_; //3D points
